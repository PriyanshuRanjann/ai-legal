...
def process_pdf_document(file_path, doc_name):
    ...
    try:
        with st.spinner(f"Processing {doc_name}..."):
            ...
            collection_name = f"collection_{doc_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        if doc_name in st.session_state.documents:
            ...
            if file_path:
                ...
                if collection_name:
                    ...
                    st.sidebar.error("Failed to process PDF - check error messages above")
            else:
...
with col2:
    if st.session_state.current_doc_id:
        st.info(f"📖 Current: {st.session_state.documents[st.session_state.current_doc_id]['name']}")
        ...
        for message in st.session_state.chat_history:
            if message['role'] == 'user':
                ...
                if 'passages' in message:
                    with st.expander(f"📚 Sources ({len(message['passages'])} passages)"):
